package org.interactivegraph.server.pidb

import java.util.{Map => JMap}

import cn.pidb.engine.{BoltService, CypherService}
...
class PidbSetting extends Setting {
  @Autowired
  ...
  def setCypherService(value: PidbService) = _cypherService = value;
...
class PidbGraphMetaDBInMemory extends GraphMetaDB with InitializingBean {
  @Autowired
  var _cypherService: CypherService = _;
